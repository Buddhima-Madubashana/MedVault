...
const DashboardHome = () => {
  ...
  const refreshUser = async () => {
    ...
    try {
      ...
      // Check for active admin permission or emergency override
      ...
      } else if (data.overrideExpiresAt) {
        const expiry = new Date(data.overrideExpiresAt).getTime();
        const now = Date.now();
        if (expiry > now) {
          setTimeLeft(expiry - now);
        } else {
    ...
    <div className="space-y-8">
      ...
      <div className="relative p-8 overflow-hidden text-white shadow-xl bg-gradient-to-r from-primary-600 to-teal-600 dark:from-primary-800 dark:to-teal-800 rounded-3xl shadow-primary-900/20">
        <div className="relative z-10 flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          ...
          <div className="flex gap-3">
             ...
             {/* Emergency Override badge shown in the header */}
             {!isTempAdmin && currentUser?.overrideExpiresAt && (
               <div className="flex items-center gap-2 px-5 py-2.5 bg-rose-500/20 border border-rose-200/50 rounded-xl">
                 <Shield className="animate-pulse text-rose-200" size={20} />
                 <span className="font-bold text-rose-50">Emergency Access Active · {formatTimeKey(timeLeft)}</span>
               </div>
             )}
      ...
      <div className="flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          {/* Calendar placed under Ward & Patient Statistics for all roles */}
          {renderCalendar()}
