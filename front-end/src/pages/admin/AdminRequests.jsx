...
const AdminRequests = () => {
  ...
  if (user?.isTempAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] p-8 text-center bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm mt-8">
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-blue-50 bg-blue-50/50">
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <Clock className="text-blue-500" /> Pending Requests
        ...
        <div className="divide-y divide-slate-100">
          {pendingRequests.length === 0 ? (
            ...
            pendingRequests.map((req) => (
              <div key={req._id} className="p-6 flex items-start justify-between">
                <div>
                  ...
                  <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 mt-2 max-w-xl">
                    ...
                    <p className="text-sm text-slate-700 mt-1">
                      <span className="font-semibold">Duration:</span> {req.duration}{" "}
                      minutes
                    </p>
                  </div>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 font-bold transition shadow-sm"
                  >
                    <CheckCircle size={18} /> Approve
                  </button>
                  <button
                    onClick={() => handleAction(req._id, "reject")}
                    className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-50 font-bold transition shadow-sm"
                  >
      ...
      <div className="bg-white rounded-2xl border border-green-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-green-50 bg-green-50/50">
          <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              ...
              <div key={req._id} className="p-6 flex items-start justify-between">
                ...
                <div>
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 border border-red-100 rounded-lg hover:bg-red-100 font-bold transition"
                  >
